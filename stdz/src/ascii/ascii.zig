...
//usando comptime do tipo type posso passar quaisquer tipo
//para os parametros.
inline fn is_odd(comptime T: type, x: T) bool {
...
//usando comptime do tipo type posso passar quaisquer tipo
//para os parametros.
inline fn is_even(comptime T: type, x: T) bool {
...
test "basic is_uppercase func" {
    try std.testing.expect(is_uppercase('T') == true);
}
...
inline fn is_lowercase(x: u8) bool {
    return if (x >= 'a' and x <= 'z') true else false;
    //código alternativo
    //return switch(x){'a'...'z'=> true, else => false,};
}
test "basic is_lowercase func" {
    try std.testing.expect(is_lowercase('m') == true);
}

//Nome: is_alphabetic(x) => se x>= a e x<= z ou
//                             x>= A e x<= Z retorna true senão retorna false
//Função: Verifica se x é do ALFABETO
//Retorna: se sim true senão false.
//Observação: .
inline fn is_alphabetic(x: u8) bool {
    return switch (x) {
        'a'...'z', 'A'...'Z' => true,
        else => false,
    };
}
test "basic is_alphabetic func" {
    try std.testing.expect(is_alphabetic('M') == true);
}
...
//Retorna: se sim true senão false.
//Observação: .
inline fn is_alphanumeric(x: u8) bool {
    return switch (x) {
        'a'...'z', 'A'...'Z', '0'...'9' => true,
        else => false,
    };
}
test "basic is_alphanumeric func" {
    try std.testing.expect(is_alphanumeric('M') == true);
}

//Nome: is_blank(x) => se x = (' ','\t','\r','\n') retorna true senão retorna false
//Função: Verifica se a letra em x (space, newline, tab, carriege return)
//Retorna: se sim true senão false.
//Observação: .
...
//Nome: is_hexadecimal(x) => se is_digit(x) ou x>='A' e x<='F' retorna true senão retorna false
//Função: Verifica se x é um valor HEXADECIMAL
//Retorna: se sim true senão false.
//Observação: .
inline fn is_hexadecimal(x: u8) bool {
    //não fecha todos as possibilidades
    //return if (is_digit(x) or (to_uppercase(x) >= 'A' and to_uppercase(x) <= 'F')) true else false;
    //trocado por:
    return switch (x) {
        'A'...'F', 'a'...'f', '0'...'9' => true,
        else => false,
...
test "basic is_hexadecimal func" {
    try std.testing.expect(is_hexadecimal('f') == true);
}

//Nome: is_octal(x) => se x>='0' e x<='7' retorna true senão retorna false
//Função: Verifica se x é um valor OCTAL
//Retorna: se sim true senão false.
//Observação: .
inline fn is_octal(x: u8) bool {
    return if (x >= '0' and x <= '7') true else false;
}
test "basic is_OCTAL func" {
    try std.testing.expect(is_octal('7') == true);
}
